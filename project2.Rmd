...
```{r}
sharesStats <- expandChannelData %>% summarise(Min = min(shares), Median = median(shares), Mean = mean(shares), q98 = quantile(shares, 0.98), Max = max(shares), "# Articles in Top 2%" = sum(busData$shares > quant98$q98, na.rm = TRUE), "# Total Articles" = length(shares))
```
...
outliers <- expandChannelData %>% filter(shares > 200000) %>% select(shares, channel, title)
outliers
